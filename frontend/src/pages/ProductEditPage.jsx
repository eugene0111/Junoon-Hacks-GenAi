...
const ProductFormFields = ({ initialData, onSubmit }) => {
    ...
    const [formData, setFormData] = useState({
        name: '', description: '', price: '', category: 'Other', status: 'draft',
    ...
    const handleSuggestPrice = async () => {
        if (!formData.name || !formData.category || !formData.description) {
            alert('Please provide a Name, Category, and Description before suggesting a price.');
            return;
        }
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-10 rounded-2xl shadow-lg space-y-8 border border-gray-200">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label htmlFor="price" className="block text-sm font-bold text-gray-700">Price (USD)</label>
                             <button
                                type="button"
                        ...
                        {priceSuggestion && (
                            <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm">
                                ...
                                <p className="text-yellow-700">{priceSuggestion.justification}</p>
                            </div>
                        )}
                    </div>
            ...
            <div className="space-y-6">
                ...
                <div className="mt-1"><FormInput label="Primary Image URL" id="images.0.url" name="images.0.url" type="text" value={formData.images[0].url} onChange={handleChange} placeholder="https://example.com/image.png" /></div>
            </div>
            ...
            <div className="flex justify-end items-center gap-4 pt-5 border-t border-gray-200">
                ...
                <button type="submit" disabled={formLoading} className="bg-google-blue text-white font-bold px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:bg-blue-300 disabled:cursor-not-allowed">{formLoading ? 'Saving...' : (initialData ? 'Update Product' : 'Create Product')}</button>
            </div>
        </form>
    );
};
...
const ProductEditPage = () => {
    ...
    if (pageLoading) {
        return <div className="flex justify-center items-center h-screen"> <h2 className="text-xl font-semibold">Loading Product...</h2> </div>;
    }
    if (pageError) {
        return <div className="flex justify-center items-center h-screen"> <p className="text-xl text-red-600">{pageError}</p> </div>;
    }
    ...
    return (
        <>
            ...
            <main className="pt-24 bg-gray-50 font-sans container mx-auto px-6 py-16 min-h-screen">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-gray-600">
                                    {isEditMode 
                                        ? `You are currently editing "${initialData?.name || 'product'}".`
                ...
                <AnimatedSection>
                    <ProductFormFields initialData={initialData} onSubmit={handleFormSubmit} />
                </AnimatedSection>
            </main>
            <Footer />
        </>
